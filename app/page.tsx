...
export default async function Page() {
  ...
  const [{ data: items }, { data: projects }, { data: activity }, { data: history }, { data: work }, { data: workDays }] = await Promise.all([
    ...
    supabase.from('lifequest_work').select('id, month, monthly_salary').eq('user_id', user.id).eq('month', monthKey).maybeSingle(),
    supabase.from('lifequest_work_days').select('worked_on').eq('user_id', user.id).gte('worked_on', monthStart),
  ])
  const monthLabel = new Date(`${monthKey}-01T00:00:00Z`).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric', timeZone: 'UTC' })
  return <LifeQuestDashboard authenticatedEmail={user.email ?? ''} profile={profile} items={items ?? []} projects={projects ?? []} activity={activity ?? []} history={history ?? []} work={work ?? null} workDaysCount={(workDays ?? []).length} alreadyMarkedToday={(workDays ?? []).some((d) => d.worked_on === todayKey)} monthLabel={monthLabel} />
}
