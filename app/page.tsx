...
export default async function Page() {
  ...
  const [{ data: items }, { data: projects }, { data: activity }] = await Promise.all([
  ...
  return <LifeQuestDashboard authenticatedEmail={user.email ?? ''} profile={profile} items={items ?? []} projects={projects ?? []} activity={activity ?? []} />
}
